...
- Player list models include `identities`, so migrated QQ/Aime/scan bindings can be shown without a second detail lookup.
...
- Pricing APIs use the backend `time.priority` provider shape: rules are sent under `provider.rules`, time ranges live in `timeRange` or migrated `dateTimeRange`, and price fields live in `pricing`. Timeline previews read backend `segments` and render them as staff-facing day segments.
