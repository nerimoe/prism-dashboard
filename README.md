...
- The selected player's bill keeps the compact `pricingCharges` summary and adds `pricingSegments` for the actual charged ISO time range, matched rule range, amount, and an interval-cap badge only after that interval has reached its cap. `globalCapWindows` are grouped by their anchored cap window, expandable to the complete time range and participating items; a reached window shows the capped final amount, while an unreached window shows its current included amount rather than a discount delta.
...
- Pricing APIs use the backend `time.priority`, `time.cap`, and `charge.fixed` shapes directly. Time rules are sent under `provider.rules`, time ranges live in `timeRange` or migrated `dateTimeRange`, and price fields live in `pricing`. Global cap timelines send `provider.includedPricingConfigIds` plus cap rules whose `priceCap` sits on the rule itself, while preserving the edited `timeRange` instead of falling back to full-day `00:00-00:00`. Fixed charges send `provider.label` and `provider.amount`. Timeline previews read backend `segments` and render the day ring from backend `startMinute` / `endMinute`; `time.cap` previews use the same ring and display each segment's cap amount, so cross-day and migrated date-scoped rules stay aligned with settlement behavior. Existing pricing configs keep their original type when edited; staff must click `新建方案` to add a separate global cap timeline.
