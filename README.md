...
- Asset management dialogs keep staff on configured business objects: asset category is selected from known categories, present grants select an active asset definition, and redeem codes select an active present instead of asking for backend IDs.
...
- Pricing APIs use the backend `time.priority` provider shape: rules are sent under `provider.rules`, time ranges live in `timeRange` or migrated `dateTimeRange`, and price fields live in `pricing`. Timeline previews read backend `segments` and render them as a day ring plus staff-facing segment chips.
