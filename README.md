...
- Pricing APIs use the backend `time.priority`, `time.cap`, and `charge.fixed` shapes directly. Time rules are sent under `provider.rules`, time ranges live in `timeRange` or migrated `dateTimeRange`, and price fields live in `pricing`. Global cap timelines send `provider.includedPricingConfigIds` plus cap rules whose `priceCap` sits on the rule itself. Fixed charges send `provider.label` and `provider.amount`. Timeline previews read backend `segments` and render the day ring from backend `startMinute` / `endMinute`; `time.cap` previews use the same ring and display each segment's cap amount, so cross-day and migrated date-scoped rules stay aligned with settlement behavior.
